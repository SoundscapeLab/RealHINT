function [snr50_Cafe1_2] = RealHINT_EHF2251F_Cafe1_2(start, sentences, subject_num)
%%%%%%%%%%%%%%%%%%%%%%%%%
%
%%%%%%%%%%%%%%%%%%%%%%%%%

addpath("BKB_EHF2251_female_cafe1");
...
load RealHINT_EHF2251F.mat BKBCafe1_EHF2251F BKBSentences Lnoise % get the  info

Lnoise = Lnoise.Cafe1;

RealHINTCafe1_Scores = zeros(1,20);
RealHINTCafe1_SNRs = zeros(1,20);

targets = sentences(start:start+19); % 

uiwait(msgbox('CAFE 1'));

% double check participant
waitfor(msgbox("Please make sure the participant is sitting up straight.", "Warning", "warn"));
waitfor(msgbox("Start Cafe 1 (Cafe1_8_B36Cal) Noise in a Random Spot", "Warning", "warn"));
...
for ii = 1:4
    ...
    sentenceFile = char(BKBCafe1_EHF2251F{targets(ii),1});
    ...
    RealHINTCafe1_Scores(ii) = answer;
    RealHINTCafe1_SNRs(ii) = snr;
...
% calculate SNR50
snr50_Cafe1_2 = mean(RealHINTCafe1_SNRs(4:20));

% save data
nameSave = sprintf('Subject_%3d_RealHINT_EHF2251F_Cafe1_2', subject_num);
% Generate text file
...
fprintf(fid,'%s %d\r\n','Subject ', subject_num);
fprintf(fid,'%s %.2f\r\n','SNR50 = ',snr50_Cafe1_2);
fprintf(fid,'%s %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f\n','SNRs = ', RealHINTCafe1_SNRs);
fprintf(fid,'%s %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d\n','TRACING = ', RealHINTCafe1_Scores);
fclose(fid);

waitfor(msgbox("Stop Cafe 1 (Cafe1_8_B36Cal) Noise", "Warning", "warn"));

% display SNR
message = sprintf('The SNR50 is %.2f dB', snr50_Cafe1_2);
