function [snr50_Diffuse_1] = RealHINT_EHF2251F_Diffuse_1(start, sentences, subject_num)
%%%%%%%%%%%%%%%%%%%%%%%%%
%
%%%%%%%%%%%%%%%%%%%%%%%%%

addpath("BKB_EHF2251_female_Diffuse");
...
load RealHINT_EHF2251F.mat BKBDiffuse_EHF2251F BKBSentences Lnoise % get the  info

Lnoise = Lnoise.Diffuse;

RealHINTDiffuse_Scores = zeros(1,20);
RealHINTDiffuse_SNRs = zeros(1,20);

targets = sentences(start:start+19); %

uiwait(msgbox('DIFFUSE'));

% double check participant
waitfor(msgbox("Please make sure the participant is sitting up straight.", "Warning", "warn"));
waitfor(msgbox("Start Diffuse (Diffuse_8_B36Cal) Noise in a Random Spot", "Warning", "warn"));
...
for ii = 1:4
    ...
    sentenceFile = char(BKBDiffuse_EHF2251F{targets(ii),1});
    ...
    RealHINTDiffuse_Scores(ii) = answer;
    RealHINTDiffuse_SNRs(ii) = snr;
...
% calculate SNR50
snr50_Diffuse_1 = mean(RealHINTDiffuse_SNRs(4:20));

% save data
nameSave = sprintf('Subject_%3d_RealHINT_EHF2251F_Diffuse_1', subject_num);
% Generate text file
fid = fopen([nameSave '_info.txt'],'w');
fprintf(fid,'%s %s\r\n','File generated on: ', datetime("now"));
fprintf(fid,'%s %.2f\r\n','Environment Check Lnoise ', Lnoise);
fprintf(fid,'%s %d\r\n','Subject ', subject_num);
fprintf(fid,'%s %.2f\r\n','SNR50 = ',snr50_Diffuse_1);
fprintf(fid,'%s %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f\n','SNRs = ', RealHINTDiffuse_SNRs);
fprintf(fid,'%s %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d\n','TRACING = ', RealHINTDiffuse_Scores);
fclose(fid);

waitfor(msgbox("Stop Diffuse (Diffuse_8_B36Cal) Noise", "Warning", "warn"));

% display SNR
message = sprintf('The SNR50 is %.2f dB', snr50_Diffuse_1);
