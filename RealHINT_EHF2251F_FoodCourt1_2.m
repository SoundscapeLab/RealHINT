function [snr50_FoodCourt1_2] = RealHINT_EHF2251F_FoodCourt1_2(start, sentences, subject_num)
%%%%%%%%%%%%%%%%%%%%%%%%%
%
%%%%%%%%%%%%%%%%%%%%%%%%%

addpath("BKB_EHF2251_female_FoodCourt1");
...
load RealHINT_EHF2251F.mat BKBFoodCourt1_EHF2251F BKBSentences Lnoise % get the  info

Lnoise = Lnoise.FoodCourt1;

RealHINTFoodCourt1_Scores = zeros(1,20);
RealHINTFoodCourt1_SNRs = zeros(1,20);

targets = sentences(start:start+19); % 

uiwait(msgbox('FOOD COURT 1'));

% double check participant
waitfor(msgbox("Please make sure the participant is sitting up straight.", "Warning", "warn"));
waitfor(msgbox("Start FoodCourt 1 (FoodCourt1_8_B36Cal) Noise in a Random Spot", "Warning", "warn"));
...
for ii = 1:4
    ...
    sentenceFile = char(BKBFoodCourt1_EHF2251F{targets(ii),1});
    ...
    RealHINTFoodCourt1_Scores(ii) = answer;
    RealHINTFoodCourt1_SNRs(ii) = snr;
...
% calculate SNR50
snr50_FoodCourt1_2 = mean(RealHINTFoodCourt1_SNRs(4:20));

% save data
nameSave = sprintf('Subject_%3d_RealHINT_EHF2251F_FoodCourt1_2', subject_num);
% Generate text file
fid = fopen([nameSave '_info.txt'],'w');
fprintf(fid,'%s %s\r\n','File generated on: ', datetime("now"));
fprintf(fid,'%s %.2f\r\n','Environment Check Lnoise ', Lnoise);
fprintf(fid,'%s %d\r\n','Subject ', subject_num);
fprintf(fid,'%s %.2f\r\n','SNR50 = ',snr50_FoodCourt1_2);
fprintf(fid,'%s %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f\n','SNRs = ', RealHINTFoodCourt1_SNRs);
fprintf(fid,'%s %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d\n','TRACING = ', RealHINTFoodCourt1_Scores);
fclose(fid);

waitfor(msgbox("Stop FoodCourt 1 (FoodCourt1_8_B36Cal) Noise", "Warning", "warn"));

% display SNR
message = sprintf('The SNR50 is %.2f dB', snr50_FoodCourt1_2);
